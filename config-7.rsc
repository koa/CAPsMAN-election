...
remove [find]
add name=default-3 originate-default=never router-id=(172.16.0.0+$number) version=3  redistribute=connected,modem
add name=default-2 originate-default=never router-id=(172.16.0.0+$number) version=2  redistribute=connected,modem
/routing ospf area
remove [find]
add instance=default-2 name=backbone_v2
add instance=default-3 name=backbone_v3
...
remove [find]
add area=backbone_v2 interfaces=loopback passive
add area=backbone_v3 interfaces=loopback passive
add area=backbone_v2 interfaces=[/interface/ethernet/find]
add area=backbone_v3 interfaces=[/interface/ethernet/find]
...
add dont-require-permissions=no name=check-master owner=admin policy=ftp,reboot,read,write,policy,test,password,sniff,sensitive source="       {\
    ...
    \n                   #:put \$masterIP\
    \n                   :local len [:len [/ipv6 route find where dst-address=\$masterIP]]\
    \n                   #:put \$len\
    \n                   :set \$masterCount (\$masterCount+[\$len])\
    \n               }\
    \n           }\
    \n           :local isMaster 0\
    ...
    \n                   /interface wireless cap set caps-man-addresses=\"\" discovery-interfaces=eoipv6-master-tunnel enabled=yes interfaces=[/interface wireless find where interface-type!=virtual] certificat\
    e=none\
    ...
    \n           #:put \"interface\"\
    \n           #:put \$interfName\
    \n           :foreach neighbor in=[/routing/ospf/neighbor/find where address~(\"%\".\$interfName)] do={\
    \n             #:put \$neighbor\
    \n             :local neighborId [/routing ospf neighbor get value-name=router-id \$neighbor]\
    \n             #:put \$neighborId\
    \n             :local myId [/routing ospf instance get value-name=router-id [/routing/ospf/instance/find where version=3]]\
